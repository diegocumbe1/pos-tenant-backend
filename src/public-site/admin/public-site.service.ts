...
export class PublicSiteService {
  ...
  async getPublicSiteBySlug(slug: string) {
    ...
    const payload = site.publishedPayload as Record<string, unknown>;
    // El snapshot puede venir de una publicación anterior con secciones que la
    // vertical ya no renderiza; se filtran al servir para no obligar a
    // republicar cada sitio.
    const unsupported = this.strategies.unsupportedSectionTypes(
      site.tenant.vertical?.code,
    );
    const sections = Array.isArray(payload.sections)
      ? (payload.sections as Array<{ type?: string }>).filter(
          (section) => !unsupported.has(section.type ?? ''),
        )
      : payload.sections;
  ...
  private toSiteDto(
    ...
    return {
      ...
      sections: site.sections
        ...
        // Fuera las secciones que la vertical no renderiza (una tienda con
        // "Servicios" heredado de una semilla anterior). Evita migrar datos.
        .filter((section) => !unsupported.has(section.type))
        .map((section) => ({
          id: section.id,
          type: section.type,
          isVisible: section.isVisible,
  ...
  private async uploadFile(
    ...
    return this.imageUpload.uploadImage({
      ...
      pathPrefix: `tenants/${ctx.tenantId}/public-site/${siteId}/${kind}`,
    });
  }
