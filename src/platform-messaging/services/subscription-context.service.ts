...
export class SubscriptionContextService {
  ...
  async build(tenantId: string, now = new Date()): Promise<SubscriptionContext> {
    const tenant = await this.prisma.tenant.findUnique({
      where: { id: tenantId },
      include: { subscription: true },
    });
    if (!tenant) throw new NotFoundException(`Tenant no encontrado: ${tenantId}`);
    ...
    if (sub && !dueAt) missing.push('la suscripción no tiene fecha de vencimiento');
    ...
    const suspendsAt = sub?.graceEndsAt ?? (dueAt ? addDays(dueAt, GRACE_DAYS) : null);
    const amountCOP = sub?.agreedPriceCOP ?? sub?.priceCOP ?? sub?.listPriceCOP ?? 0;
    ...
    return {
      ...
      planLabel: PLAN_LABELS[sub?.plan ?? tenant.plan] ?? (sub?.plan ?? tenant.plan),
  ...
  toVariables(ctx: SubscriptionContext): Record<string, string> {
    ...
    return {
      ...
      dias_para_vencer: ctx.daysToDue !== null ? String(Math.max(0, ctx.daysToDue)) : '',
      dias_vencido: ctx.daysToDue !== null ? String(Math.max(0, -ctx.daysToDue)) : '',
      fecha_suspension: ctx.suspendsAt ? formatDateCO(ctx.suspendsAt) : '',
      fecha_suspension_larga: ctx.suspendsAt ? formatDateLongCO(ctx.suspendsAt) : '',
      dias_de_holgura:
