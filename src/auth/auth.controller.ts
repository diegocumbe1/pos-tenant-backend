...
export class AuthController {
  ...
  async signupInvite(@Body() dto: SignupInviteDto) {
    ...
    const verticalCode = this.normalizeVertical(dto.vertical);
    ...
    const menuSlug = await this.nextAvailableMenuSlug(
      this.slugify(businessName),
    );
    ...
    const tenant = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.menuPublicConfig.create({
        data: {
          ...
          featuredProductIds: [],
          categoryOrder: [],
          ...this.defaultMenuTheme(businessName),
        },
      });
  ...
  async me(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    ...
    timings.profile = this.elapsed(profileStart);
    ...
    timings.permissions = this.elapsed(permissionsStart);

    const branchesStart = performance.now();
    const branches = await this.resolveAccessibleBranches(user);
    timings.branches = this.elapsed(branchesStart);
    timings.total = this.elapsed(totalStart);

    res.setHeader('Cache-Control', 'no-store');
    res.setHeader('Server-Timing', this.toServerTimingHeader(timings));
    this.logger.log(
      `me timing userId=${authUser.id} profile=${timings.profile.toFixed(
        1,
      )}ms permissions=${timings.permissions.toFixed(1)}ms branches=${timings.branches.toFixed(
        1,
      )}ms total=${timings.total.toFixed(1)}ms`,
    );
  ...
  async login(
    ...
    timings.auth = this.elapsed(authStart);
    ...
    res.setHeader('Server-Timing', this.toServerTimingHeader(timings));
    this.logger.log(
      `login timing email=${dto.email} userId=${authUser.id} auth=${timings.auth.toFixed(
        1,
      )}ms profile=${(timings.profile ?? 0).toFixed(1)}ms branches=${(
        timings.branches ?? 0
      ).toFixed(1)}ms total=${timings.total.toFixed(1)}ms`,
    );
  ...
  private async buildLoginResponse(
    ...
    if (timings) timings.profile = this.elapsed(profileStart);
    ...
    const branchesStart = performance.now();
    const branches = await this.resolveAccessibleBranches(user);
    if (timings) timings.branches = this.elapsed(branchesStart);
    ...
    if (timings) timings.permissions = this.elapsed(permissionsStart);
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Send a password recovery email via Supabase.',
  })
  ...
  private async loadLoginProfile(userId: string) {
  ...
  private defaultMenuTheme(tenantName: string) {
    return {
      templateId: 'cards',
      primaryColor: '#16a34a',
      accentColor: '#f97316',
