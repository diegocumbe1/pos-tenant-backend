...
export class StorageRemoteAuthStore {
  ...
  private get objectPath(): string {
    return `whatsapp-sessions/${this.clientId}.zip`;
  }
  ...
  async save(options: { session: string }): Promise<void> {
    ...
    // RemoteAuth invoca este save() desde un setInterval sin catch: si aquí se
    // lanza, es un unhandled rejection que tumba el proceso. Un backup fallido
    // solo debe registrarse — el siguiente ciclo lo reintenta.
    try {
